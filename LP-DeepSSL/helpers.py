...
def create_data_loaders(train_transformation,
    ...
    if not args.fully_supervised and args.labels:
        ...
        with open(args.labels) as f:
            for line in f.read().splitlines():
                ...
                # img = imagePath = os.path.join('/media/luckie/vol4/semi_supervised_cls/dataset/skin/training_data/',
                #  lineItems[0])
                img = lineItems[0]
                label = lineItems[1:].index(max(lineItems[1:]))
    ...
    # eval_dataset =   DatasetGenerator(pathImageDirectory='/media/luckie/vol4/semi_supervised_cls/dataset/skin/training_data', 
    #     pathDatasetFile='/media/luckie/vol4/semi_supervised_cls/dataset/skin/validation.txt',
...
def train(train_loader, model, optimizer, epoch, global_step, args, ema_model = None):
    ...
    class_criterion = nn.CrossEntropyLoss( ignore_index=NO_LABEL, reduction='none',
...
def load_args(args, isMT = False):
    ...
    if args.dataset == "cifar100":
        ...
        #args.labels = '%s/labels/%s/%d_balanced_labels/%d.txt' % (label_dir,args.dataset,args.num_labeled,args.label_split)
        args.labels = "/media/luckie/vol4/semi_supervised_cls/dataset/skin/train_labeled.txt"
...
def epochTest(args, wandb, model, epoch, dataloader):
        ...
        for i, (input,target) in enumerate(dataloader):
            ...
            # target = torch.zeros(target.shape[0], 7).scatter_(1, target, 1)
            target = target.cuda()
            outGT = torch.cat((outGT, target), 0)
            bs, n_crops, c, h, w = input.size()
            with torch.no_grad():
                out,_ = model(input.view(-1, c, h, w).cuda())
