...
def make_dataset_new(dir, class_to_idx, extensions):
    images = []

    fileDescriptor = open(dir, "r")
    ...
    while line:
        ...
        if line:
            ...
            label = imageLabel.index(max(imageLabel))
            item = (imagePath,label)
            images.append(item)
    return images
...
class DatasetFolder(data.Dataset):
    ...
    def __init__(self, pathDatasetFile, loader, extensions, transform=None, target_transform=None):
        ...
        if len(self.listImagePaths) == 0:
            raise(RuntimeError("Found 0 files in image list"))
    ...
    def __getitem__(self, index):
        ...
        sample = Image.open(imagePath).convert('RGB')
        target = torch.FloatTensor(self.listImageLabels[index])

        if self.transform != None: imageData = self.transform(sample)
        weight = self.p_weights[index]
        c_weight = self.class_weights[target]
...
class DBSS(DatasetFolder):
    ...
    def __init__(self, root, transform=None, target_transform=None,
        ...
        super(DBSS, self).__init__(root, loader,
                                          transform = transform,
